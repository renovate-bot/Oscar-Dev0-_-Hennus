import { APIAllowedMentions, RESTPatchAPIChannelMessageJSONBody, RESTPostAPIChannelMessageJSONBody } from "@discordjs/core";
import { MessageOptions, Client, ModelsBase, MessageChannelCreate, _Omit } from "..";
...
export class Message extends ModelsBase<MessageOptions> {
    ...
    async delete(reason?: string){
        try {
            await this.client.api.channels.deleteMessage(this.channelID, this.id, {reason});
            return this;
        } catch (error) {
            throw error;
        };
    ...
    async edit(options: MessageChannelCreate){
        try {
            ...
            const msg = await this.client.api.channels.editMessage(this.channelID, this.id, data);
            const format = new Message(this.client, msg);
            return format;
        } catch(error) {
            throw error;
    ...
    async pin(reason?: string){
        try {
            await this.client.api.channels.pinMessage(this.channelID, this.id, {reason});
            return this;
        } catch (error) {
            throw error;
        };

...
import { Client, BaseGuildChannel, MessageChannelCreate, Message, MessagesManager} from "..";
...
export class GuildTextChannel extends BaseGuildChannel<APITextChannel | APINewsChannel> {
    ...
    get messages(){
        if(!this._messages) {this._messages = new MessagesManager(this.client); this._messages.fetchAll(this.id)};

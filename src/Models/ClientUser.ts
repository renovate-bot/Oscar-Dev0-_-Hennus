import { GatewayOpcodes, GatewayReadyDispatchData, GatewaySendPayload, GatewayUpdatePresence, PresenceUpdateStatus } from "@discordjs/core";
import { Client } from "../client";
import { User } from "./user";
import { ApplicationBitField } from "../utils";
import { _Omit } from "../types";

export interface Activity extends _Omit<GatewayUpdatePresence["d"], "since" | "afk" | "status"> {
...
export class ClientUser extends User {
    ...
    constructor(client: Client, data: GatewayReadyDispatchData) {
        ...
        this.applicationFlags = new ApplicationBitField(data.application.flags).freeze();
    };

    async setActivity(status: Activity, shards?: number | number[]) {
        const data: GatewaySendPayload = { op: GatewayOpcodes.PresenceUpdate, d: { since: null, activities: status.activities, afk: false, status: this.format(status.status) } };
        //@ts-ignore
        if (Array.isArray(shards)) { for (let shard of shards) { try { await this.client.ws.send(shard, data) } catch { continue; } } }
        //@ts-ignore
    ...
    private format<T extends Activity["status"]>(status: T): PresenceUpdateStatus {

...
export class ChannelsManager extends DataManager<string, Channel> {
  ...
  async fetchGuild(guildId: string) {
    const channels = (await this.client.api.guilds.getChannels(guildId)).map(
      (x) => ChannelResolved(this.client, x),
    );
  ...
  async create(guildId: string, options: RESTPostAPIGuildChannelJSONBody) {
    const channel = await this.client.api.guilds.createChannel(
      guildId,
      options,
    );

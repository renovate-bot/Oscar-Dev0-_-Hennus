...
export class GuildMembersManager extends DataManager<string, GuildMember> {
  ...
  constructor(client: Client, guild: Guild, map?: GuildMember[]) {
    const _map: Iterable<readonly [string, GuildMember]> | undefined = map
      ?.filter((x) => x.user != undefined)
      .map((x) => [x.user?.id ?? "", x]);
    super(client, _map);
  ...
  async fetch(guildId: string, GuildMemberId: string, force?: boolean) {
    ...
    const data = await this.client.api.guilds.getMember(guildId, GuildMemberId);
    if (data) {
  ...
  async fetchAll(guildId: string, options?: RESTGetAPIGuildMembersQuery) {
    const GuildMembers = (
      await this.client.api.guilds.getMembers(guildId, options)
    )
      ?.filter((X) => X.user != undefined)
      .map((x) => new GuildMember(this.client, x, this.guild));
    if (Array.isArray(GuildMembers))
      this.setCache(GuildMembers.map((x) => [x.user?.id ?? "", x]));
    return GuildMembers;
  }
}

...
export class MessagesManager extends DataManager<string, Message> {
    constructor(client: Client, map?: Message[]){
        const _map: Iterable<readonly [string, Message]> | undefined = map?.map((x)=>([x.id, x]))
        super(client, _map);
    };
    ...
    async fetch(channelId: string, MessageId: string, force?: boolean){
        ...
        const data = await this.client.api.channels.getMessage(channelId, MessageId);
    ...
    async fetchAll(channelId: string, options?: RESTGetAPIChannelMessagesQuery) {
        const messages = await this.client.api.channels.getMessages(channelId, options)
        for(const _message of messages) {
            const message = new Message(this.client, _message);

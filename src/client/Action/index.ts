...
export class ActionEvents {
  ...
  payload(client: Client, data: GatewayDispatchPayload) {
    ...
    } else if(data.t == GatewayDispatchEvents.MessageDelete) json = new MessageDelete(data, client).toData
    // Channel
    else if (
    ...
    // Ready
    else if(data.t == GatewayDispatchEvents.Ready) json  = new Ready(data, client).toData; 

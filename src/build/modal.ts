...
export class ModalBuilder {
  ...
  public addTextInputComponents(components: Array<TextInputBuilder>): this {
    for (const component of components) {
      if (
        !this.findComponentById(component.custom_id ?? "", ComponentType.Button)
      ) {
  ...
  private findComponentById(
    customId: string,
    type: ComponentType,
  ): ActionRowBuilder | undefined {
    return this.components.find((actionRow) =>
      actionRow.components.some((comp) =>
        comp.type === type && comp.custom_id === customId
      )
    );
  }
}

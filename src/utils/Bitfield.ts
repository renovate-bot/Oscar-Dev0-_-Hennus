...
export class BitField<N extends number | bigint> {
  ...
  add(...bits: BitFieldResolvable<N>[]) {
    ...
    if (Object.isFrozen(this)) {
      return new BitField<N>(
        (BigInt(this.bitfield) | total) as BitFieldResolvable<N>,
      );
    }
    //@ts-ignore
    this.bitfield |= total;
  ...
  remove(...bits: BitFieldResolvable<N>[]) {
    ...
    if (Object.isFrozen(this)) {
      return new BitField(BigInt(this.bitfield) & ~total);
    }
    //@ts-ignore
    this.bitfield &= ~total;
    return this;
  ...
  static resolve<N extends number | bigint>(bit: BitFieldResolvable<N>): N {
    ...
    if (Array.isArray(bit)) {
      return bit
        ...
        .reduce((prev, bit_) => prev | bit_, DefaultBit) as N;
    }

    if (typeof bit === "string") {
      if (!isNaN(Number(bit))) {
        return (
          typeof DefaultBit === "bigint" ? BigInt(bit) : Number(bit)
        ) as N;
      }
      if (Flags[bit as keyof typeof Flags] !== undefined) {
        return Flags[bit as keyof typeof Flags];
      }
    }
